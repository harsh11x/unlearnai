  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-bg border border-border w-full max-w-md animate-fade-up" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-8">
          <button onClick={() => { setComingSoonToast(true); setTimeout(() => setComingSoonToast(false), 2500); }} disabled={loading}
            className="w-full flex items-center justify-center gap-3 py-3 px-4 border border-border bg-bg hover:bg-surface transition-colors text-sm font-display font-medium text-text cursor-pointer disabled:opacity-50 relative">
            ...
            <span className="absolute -top-1 -right-1 bg-highlight text-white text-[8px] font-bold px-1.5 py-0.5 leading-none tracking-wider uppercase">Soon</span>
          </button>
        </div>
        ...
        {/* Coming Soon Toast */}
        {comingSoonToast && (
          <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[300] bg-accent text-accent-inv px-5 py-3 font-display font-medium text-sm flex items-center gap-2 animate-fade-up shadow-lg">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>
            GitHub sign-in coming soon — stay tuned
          </div>
        )}
      </div>
    </div>
